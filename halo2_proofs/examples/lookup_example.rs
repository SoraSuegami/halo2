...
use halo2_proofs::{
    ...
    plonk::{Advice, Circuit, Column, ConstraintSystem, Error, Expression, Instance, TableColumn},
    poly::Rotation,
};
...
impl<F: FieldExt, const N: usize> XorConfig<F, N> {
    fn generate() -> impl Iterator<Item = (F, F, F)> {
        (0..(1 << 2 * N)).scan((F::zero(), F::zero(), F::zero()), |(_, _, _), i| {
            let a_index = i & ((1 << N) - 1);
            let b_index = i >> N;
